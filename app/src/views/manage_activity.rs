...
use models::{ActivityState, ActivitySummary, ListActivityQuery, FindRecordForm, RecordEntry, RecordState};
...
fn state_controls(activity: &Binding<ActivitySummary>, state: &AppState) -> impl View {
    ...
    watch(activity.clone(), move |act: ActivitySummary| {
        ...
        vstack((
            text("Change State").font(font::Subheadline).foreground(MutedForeground),
            hstack((
                if current_state != ActivityState::NeedVolunteer {
                    AnyView::new(button(text!("Start Recruiting"))
                        .action({
                            let state = state.clone();
                            let activity = activity.clone();
                            let id = id.clone();
                            move || change_state(&state, &activity, &id, |api, id| async move { api.turn_need_volunteer(&id).await }, ActivityState::NeedVolunteer)
                        }))
                } else {
                    AnyView::new(spacer_min(0.0))
                },
                ...
                if current_state != ActivityState::Going {
                    AnyView::new(button(text!("Start Activity"))
                        .action({
                            let state = state.clone();
                            let activity = activity.clone();
                            let id = id.clone();
                            move || change_state(&state, &activity, &id, |api, id| async move { api.turn_going(&id).await }, ActivityState::Going)
                        }))
                } else {
                    AnyView::new(spacer_min(0.0))
                },
                ...
                if current_state != ActivityState::Ended {
                    AnyView::new(button(text!("End Activity"))
                        .action({
                            let state = state.clone();
                            let activity = activity.clone();
                            let id = id.clone();
                            move || change_state(&state, &activity, &id, |api, id| async move { api.turn_ended(&id).await }, ActivityState::Ended)
                        }))
                } else {
                     AnyView::new(spacer_min(0.0))
                },
                ...
                if current_state != ActivityState::Canceled {
                    AnyView::new(button(text!("Cancel"))
                        .action({
                            let state = state.clone();
                            let activity = activity.clone();
                            let id = id.clone();
                            move || change_state(&state, &activity, &id, |api, id| async move { api.turn_canceled(&id).await }, ActivityState::Canceled)
                        })
...
fn volunteer_list(
    ...
    vstack((
        text("Volunteers").font(font::Subheadline).foreground(MutedForeground),
        watch(records.clone(), {
            let state = state.clone();
...
fn volunteer_row(
    ...
    hstack((
        text(format!("User: {}", user_id.chars().take(8).collect::<String>())), // Truncate ID
        spacer(),
            ...
            AnyView::new(hstack((
                button(text!("Approve"))
                    .action({
                        ...
                        let records_binding = records_binding.clone();
                        move || {
                            update_record(&state, &record_id, &activity_id, records_binding.clone(), true)
                        }
                    }),
                button(text!("Reject"))
                    .action({
                        ...
                        let records_binding = records_binding.clone();
                        move || {
                            update_record(&state, &record_id, &activity_id, records_binding.clone(), false)
                        }
                    })
                    .foreground(Color::srgb(255, 100, 100)),
            )))
...
fn state_badge(state: ActivityState) -> impl View {
    match state {
        ...
        ActivityState::Canceled => AnyView::new(text!("Canceled").foreground(Color::srgb(255, 100, 100))),
    }
}
