...
async fn lanuch() -> Result<(), BoxError> {
    ...
    match cli.command {
        Command::Init(args) => {
            ...
            group
                .insert_one(
                    doc! {"code":"lexooo","name":"Super Admin","authority":"*"},
                    None,
                )
                .await?;
            ...
            group
                .insert_one(
                    doc! {"code":"student","name":"Student","authority":["send_comment",]},
                    None,
                )
                .await?;

            group
                .insert_one(doc! {"code":"banned","name":"Banned","authority":[]}, None)
                .await?;

            group
                .insert_one(
                    doc! {"code":"teacher","name":"Teacher","authority":["view_user",]},
                    None,
                )
                .await?;
